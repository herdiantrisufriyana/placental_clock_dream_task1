...
features <-
  c("ga", paste0("ga_res_conds_", var_ga), var_ga) |>
  `names<-`(c("ga", paste0("ga_res_conds_", var_ga), var_ga)) |>
...
non_ga_est_sets <-
  features[str_detect(names(features), "ga_res_conds")] |>
  imap(
...
non_ga_pred_sets <-
  features[
    names(features) != "ga"
    & !str_detect(names(features), "ga_res_conds")
  ] |>
  imap(
    ~ test_data_norm[, .x, drop = FALSE] |>
